 ...
 * @param {string} customMessage - Özel hata mesajı
...
export const getErrorMessage = (errorCode, customMessage = null) => {
  ...
  return ERROR_MESSAGES[errorCode] || ERROR_MESSAGES.UNEXPECTED_ERROR;
};

/**
 * Başarı mesajını al
 * @param {string} successCode - Başarı kodu
 * @param {string} customMessage - Özel başarı mesajı
 * @returns {string} Başarı mesajı
 */
export const getSuccessMessage = (successCode, customMessage = null) => {
  if (customMessage) {
    return customMessage;
  }
  
  return SUCCESS_MESSAGES[successCode] || SUCCESS_MESSAGES.CARD_CREATED;
};